...
void loop() {
  ...
  if (buttonFlag == 1) {
    ...
    if (randomOn) {
      modeCount = 0; //reset to beginning if it's in random mode
    } else {
      modeCount++;  
    }
    ...
    cp[modeCount % numModes] = CRGB( 64,   0,   0);

    // is it random time?
    if (modeCount % numModes == numModes - 1) {
      randomOn = true;
      firstRandom = true;
      cp[modeCount % numModes] = CRGB( 0,   64,   0);
    }
    else {
      randomOn = false;
      cp[modeCount % numModes] = CRGB( 64,   0,   0);
  ...
  if (randomOn) {
    if (firstRandom || (unsigned long)(millis() - randomStart) > randomLength) {
      firstRandom = false;
      modeCount = random(numModes - 1);   // pick a new mode
      randomLength = random(20,46) * 1000; // time btwn 20-45 seconds
      randomStart = millis();
    }
  ...
  if (modeCount % numModes == 0)  {
    ...
    SnowSparkle(0x3C, 0x3C, 0x3C, 20, random(100,1000));
  }  
